...
class ServicesTable
{
    public function __construct(private Request $request, private ServiceRepository $repository)
    {
    }
    ...
    private function DataTable($data)
    {
        return Datatables::of($data)->addIndexColumn()
            ->addColumn('name', function ($data) {
                if (app()->getLocale() == 'en') {
                    return "<a href='" . route('services.service.edit', $data->id) . "'>$data->name_en</a>";
                } else {
                    return "<a href='" . route('services.service.edit', $data->id) . "'>$data->name_ar</a>";
                }
            })
