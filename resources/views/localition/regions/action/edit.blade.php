...
@section('content')
    ...
    <div class="card height-auto" data-select2-id="21">
        <div class="card-body" data-select2-id="20">
            ...
            <form class="new-added-form" method="post" action="{{ route('location.regions.update', $region->id) }}"
                ...
                <div class="row" data-select2-id="18">
                    ...
                    <div class="col-12  form-group">
                        <label>{{ __('City') }} *</label>
                        <select id="city" name="city_id" class="select2 select2-hidden-accessible">
                            <option value="">Select</option>
                            @foreach ($cities as $item)
                                <option value="{{ $item->id }}" @if ($region->city->id == $item->id) selected @endif>
                                    @if (app()->getLocale() == 'en')
                                        {{ $item->name_en }}
                                    @else
                                        {{ $item->name_ar }}
                    ...
                    <div class="col-12  form-group">
                        ...
                        <select id="editstatus" name="status" class="select2 select2-hidden-accessible">
                            <option value="">Select</option>
                            <option value="{{ EnumGeneral::ACTIVE }}" @if ($region->status == EnumGeneral::ACTIVE) selected @endif>
                                {{ __(EnumGeneral::ACTIVE) }}</option>
                            <option value="{{ EnumGeneral::INACTIVE }}" @if ($region->status == EnumGeneral::INACTIVE) selected @endif>
