                ...
                <div class="row" data-select2-id="18">
                    ...
                    <div class="col-md-6  form-group">
                        ...
                        <select id="place" name="place" class="select2 select2-hidden-accessible" required>
                            ...
                            <option value="{{ EnumGeneral::HOME_BANER }}"
                                @if ($ads->place == EnumGeneral::HOME_BANER) selected @endif>
                                {{ __(EnumGeneral::HOME_BANER) }}</option>
                            <option value="{{ EnumGeneral::HOME_FLASH }}"
                                @if ($ads->place == EnumGeneral::HOME_FLASH) selected @endif>
                                {{ __(EnumGeneral::HOME_FLASH) }}</option>
                            <option value="{{ EnumGeneral::SECTORS_BANER }}"
                                @if ($ads->place == EnumGeneral::SECTORS_BANER) selected @endif>
                                {{ __(EnumGeneral::SECTORS_BANER) }}</option>
                            <option value="{{ EnumGeneral::ADS_SECREEN }}"
                                @if ($ads->place == EnumGeneral::ADS_SECREEN) selected @endif>
                                {{ __(EnumGeneral::ADS_SECREEN) }}</option>
                            <option value="{{ EnumGeneral::SEARCH_BANER }}"
                                @if ($ads->place == EnumGeneral::SEARCH_BANER) selected @endif>
                                {{ __(EnumGeneral::SEARCH_BANER) }}</option>
                            <option value="{{ EnumGeneral::SECTOR_FLASH }}"
                                @if ($ads->place == EnumGeneral::SECTOR_FLASH) selected @endif>
                                {{ __(EnumGeneral::SECTOR_FLASH) }}</option>
                            <option value="{{ EnumGeneral::SECTOR_BANER }}"
                                @if ($ads->place == EnumGeneral::SECTOR_BANER) selected @endif>
                                {{ __(EnumGeneral::SECTOR_BANER) }}</option>
                            <option value="{{ EnumGeneral::NOTIFICATION }}"
                                @if ($ads->place == EnumGeneral::NOTIFICATION) selected @endif>
                                {{ __(EnumGeneral::NOTIFICATION) }}</option>
                    ...
                    <div class="col-md-6  form-group d-none @if ($ads->sector_id) d-block @endif"
                        id="sector">
                        <label>{{ __('Sector') }} *</label>
                        <select id="" name="sector_id" class="select2 select2-hidden-accessible">
                            <option value="">{{ __('Select') }}</option>
                            @foreach ($sector as $item)
                                <option value="{{ $item->id }}" @if ($item->id == $ads->sector_id) selected @endif>
                                    {{ $item->getName() }}</option>
                            @endforeach
                        </select>
                        @error('sector_id')
                    ...
                    <div class="col-12  form-group">
                        ...
                        <select id="editstatus" name="status" class="select2 select2-hidden-accessible">
                            ...
                            @if ($ads->status == EnumGeneral::NOT_PAID)
                                <option value="{{ EnumGeneral::NOT_PAID }}"
                                    @if ($ads->status == EnumGeneral::NOT_PAID) selected @endif>
                                    {{ __(EnumGeneral::NOT_PAID) }}</option>
                            @elseif ($ads->status == EnumGeneral::APPROVED)
                    ...
                    <div class="row col-12 mb-5">
                        ...
                        @if ($ads->seller_id)
                            <div class="col-12  form-group" id="ads-post">
                                ...
                                <select id='seller' name='seller_id' class='select2 select2-hidden-accessible'
                                    required>
