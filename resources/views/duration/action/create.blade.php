...
@section('content')
    ...
    <div class="breadcrumbs-area">
        <h3>{{ __('Add Duration') }}</h3>
        <ul>
            <li>
                <a href="{{ route('dashboard') }}">{{ __('home') }}</a>
            </li>
            <li>{{ __('Add Duration') }}</li>
        </ul>
    </div>
    ...
    <div class="card height-auto" data-select2-id="21">
        <div class="card-body" data-select2-id="20">
            ...
            <form class="new-added-form" method="post" action="{{ route('offer.duration.store') }}"
                ...
                <div class="row">
                    ...
                    <div class="col-md-6  form-group">
                        ...
                        <select id="edittype" name="type" class="select2 select2-hidden-accessible">
                            <option value="">Select</option>
                            <option value="{{ EnumGeneral::DAYS }}">
                                {{ __(EnumGeneral::DAYS) }}</option>
                            <option value="{{ EnumGeneral::WEEK }}">
                                {{ __(EnumGeneral::WEEK) }}</option>
                            <option value="{{ EnumGeneral::MONTH }}">
                                {{ __(EnumGeneral::MONTH) }}</option>
                            <option value="{{ EnumGeneral::YEAR }}">
